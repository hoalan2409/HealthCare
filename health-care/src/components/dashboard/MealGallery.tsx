...
import type { MealGalleryProps } from '../../types/Dashboard';
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 w-full">
            {meals.map((meal, index) => (
              <div key={index} className="flex flex-col justify-start items-start w-full">
                ...
                {/* Meal Label Button */}
                <div className="relative w-full -mt-6 sm:-mt-7 lg:-mt-10">
                  <Button
                    className="text-[11px] sm:text-[13px] lg:text-[15px] font-normal leading-[14px] sm:leading-[16px] lg:leading-[19px] text-left text-global-1 bg-global-2 px-2 sm:px-3 lg:px-2 pt-0 pb-1 sm:pt-0 sm:pb-1.5 lg:pt-0 lg:pb-1 rounded-none h-8"
                  >
                    {meal.label}
                  </Button>
                </div>
